...
public class Spring2 : MonoBehaviour {
    ...
    void OnCollisionEnter2D(Collision2D collision) {
        ...
        for (int i = 0; i < 6; i++) { // 플레이어의 맨 앞, 중앙, 맨 뒤로 x좌표를 지정합니다.
            RaycastHit2D playerCheck = Physics2D.Raycast(new Vector2(pos.x + (0.25f * i), pos.y), GetVector(direction), 0.2f, ballLayer);
            if (playerCheck.collider != null) {
                if (playerCheck.collider.gameObject.TryGetComponent(out Rigidbody2D component)) {
                   component.velocity = new Vector2(component.velocity.x+5, component.velocity.y-2);
                }
            }
        }
    }
